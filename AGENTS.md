...
`ch.dboeckli.springframeworkguru.spring6coldmicroservice`. It consumes drink requests from Kafka
(`DrinkRequestListener`) and exposes Actuator endpoints on port `8081`.
...
- Start locally: `./mvnw spring-boot:run` (app on `:8081`, Kafka via `compose.yaml`).
...
- ITs (`Spring6ColdMicroServiceApplicationIT`, `KafkaTopicsIT`, `ActuatorInfoIT`) use `@SpringBootTest`
...
- `config/KafkaTopics` defines the topics; `health/KafkaHealthIndicator` backs the Actuator health.
...
- Deployment is Helm-only: chart in `helm-charts/`, packaged to `target/helm/repo/`, release name =
  artifactId, namespace `spring-6-cold-micro-service`.
